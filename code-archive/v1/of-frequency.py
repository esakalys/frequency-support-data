...
# Program used to plot the overfrequency frequency data
...
for scenario in scenarios:
  ...
  dsVsc = f.importData(files[1], 'data/ofMat', single=True)
  ...
  plt.grid()
  plt.title('VSC - Droop & Inertial Support - Overfrequency - Frequency')
  plt.xlabel('Time, s')
  plt.ylabel('Frequency, Hz')
  plt.xlim([0, 35])
  plt.legend(loc='lower right')
  plt.savefig(f'reportPlots/ofFrequency({scenario}).jpg', dpi=400)
