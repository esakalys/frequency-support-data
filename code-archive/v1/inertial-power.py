...
# Program used to plot the inertial power data
scenarios = ['+5MW', '+10MW', '+20MW']
coefficients = ['10s', '20s']
...
for scenario in scenarios:
  for c in coefficients:
    ...
    dsGen = f.importData(files[1], 'data/inertialMat', single=True)
    dsVsc = f.importData(files[2], 'data/inertialMat', single=True)
    ...
    plt.grid()
    plt.title(f'VSC - Inertial Support - Generator Power - H = {c}')
    plt.xlabel('Time, s')
    plt.ylabel('Power, MW')
    plt.xlim([0, 35])
    plt.ylim([30, 80])
    plt.legend(loc='lower right')
    plt.savefig(f'reportPlots/inertialPowerGen({scenario}{c}).jpg', dpi=400)
    ...
    plt.grid()
    plt.title(f'VSC - Inertial Support - VSC Power - H = {c}')
    plt.xlabel('Time, s')
    plt.ylabel('Power, MW')
    plt.xlim([0, 35])
    plt.ylim([-5, 15])
    plt.savefig(f'reportPlots/inertialPowerVSC({scenario}{c}).jpg', dpi=400)
