...
for scenario in scenarios:
  ...
  dsGen = f.importData(files[1], 'data/diMat', single=True)
  dsVsc = f.importData(files[2], 'data/diMat', single=True)
  ...
  plt.grid()
  plt.title(f'VSC - Droop & Inertial Support - Generator Power')
  plt.xlabel('Time, s')
  plt.ylabel('Power, MW')
  plt.xlim([0, 35])
  plt.ylim([30, 80])
  plt.legend(loc='lower right')
  plt.savefig(f'reportPlots/diPowerGen({scenario}).jpg', dpi=400)
  ...
  plt.grid()
  plt.title(f'VSC - Inertial Support - VSC Power')
  plt.xlabel('Time, s')
  plt.ylabel('Power, MW')
  plt.xlim([0, 35])
  plt.ylim([-5, 20])
  plt.savefig(f'reportPlots/diPowerVSC({scenario}).jpg', dpi=400)
