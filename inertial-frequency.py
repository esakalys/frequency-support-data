...
# Program used to plot the inertial frequency data
scenarios = ['+5MW', '+10MW', '+20MW']

colors = ['#279B00', '#0089BF', '#FC2525']
for scenario in scenarios:
  files = [f'Frequency {scenario} 5s.mat',
           f'Frequency {scenario} 10s.mat',
           f'Frequency {scenario} 20s.mat']
  ...
  plt.grid()
  plt.title('VSC - Inertial Support - Frequency')
  plt.xlabel('Time, s')
  plt.ylabel('Frequency, Hz')
  plt.xlim([0, 35])
  plt.legend(loc='lower right')
  plt.savefig(f'reportPlots/inertialFrequency({scenario}).jpg', dpi=400)
